...
function App(){
  ...
  const dash=useMemo(()=>{
    ...
    const livreParaGastar=saldoInicialPix+receitaTotal-despesas-pagamentosFatura-compromissosPrevistos-reservaAporte
    const hoje=isoToday()
  ...
  const porCategoria=useMemo(()=>{
    ...
    const restante = Math.max(0, dash.receitaTotal + saldoInicialPix - arr.reduce((s,x)=>s+x.valor,0) - dash.compromissosPrevistos - dash.reservaAporte)
    if(restante > 0) arr.push({categoria:'Disponível / não gasto', valor:restante})
    return arr
  },[porCategoriaBase,dash.receitaTotal,dash.compromissosPrevistos,dash.reservaAporte,saldoInicialPix])
  ...
  const faturas=useMemo(()=>{
    return CONTAS.map(conta=>({conta, valor:transacoes.filter(t=>t.conta===conta&&t.forma==='Crédito'&&['Despesa','Investimento','Pagamento Fatura'].includes(t.tipo)).reduce((s,t)=>s+Number(t.valor),0)}))
  },[transacoes])
  ...
  function exportarCSV(){
    ...
    a.href=url; a.download=`controle-gastos-${inicioCiclo}-a-${fimCiclo}.csv`; a.click()
  }
  ...
  return <div className="app">
    <header className="topbar"><div><h1>Controle de Gastos</h1><p>Ciclo: {formatBR(inicioCiclo)} até {formatBR(fimCiclo)} • {session.user.email}</p></div><button className="ghost" onClick={()=>supabase.auth.signOut()}><LogOut size={16}/> Sair</button></header>
    <main>
      ...
      <section className="cards">
        ...
        <div className="card hero-card"><span>Livre para gastar</span><strong className={dash.livreParaGastar>=0?'green':'red'}>{money(dash.livreParaGastar)}</strong></div>
      </section>

      <section className="cards">
        <div className="card"><span>Despesas realizadas</span><strong className="red">{money(dash.despesas+dash.pagamentosFatura)}</strong></div>
